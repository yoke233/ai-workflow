...
func runBuiltinSCMOpenPR(ctx context.Context, store core.Store, bus core.EventBus, tokens GitHubTokens, step *core.Step, execRec *core.Execution) error {
	if store == nil {
	...
	title := fmt.Sprintf("ai-flow v2: flow %d", step.FlowID)
	body := "Automated change request created by ai-workflow v2."
	if step.Config != nil {
	...
	providers := newChangeRequestProviders(token)
	...
	var fallbackProvider ChangeRequestProvider
	if mergeToken != "" && mergeToken != strings.TrimSpace(token) {
		fallbackProviders := newChangeRequestProviders(mergeToken)
		fallbackProvider, _, _, _ = detectChangeRequestProvider(ctx, originURL, fallbackProviders)
	}
	...
	cr, created, err := provider.EnsureOpen(ctx, repo, EnsureOpenInput{
		Head:  headBranch,
		Base:  baseBranch,
		Title: title,
		Body:  body,
	...
	if err != nil {
		...
		if fallbackProvider != nil && isAuthError(err) {
			cr2, created2, err2 := fallbackProvider.EnsureOpen(ctx, repo, EnsureOpenInput{
				Head:  headBranch,
				Base:  baseBranch,
				Title: title,
				Body:  body,
