...
import (
	...
	"github.com/yoke233/ai-workflow/internal/plugins/workspace-clone"
	"golang.org/x/oauth2"
)
...
func (p *GitHubProvider) Detect(_ context.Context, originURL string) (ChangeRequestRepo, bool, error) {
...
func (p *GitHubProvider) EnsureOpen(ctx context.Context, repo ChangeRequestRepo, input EnsureOpenInput) (ChangeRequest, bool, error) {
	if strings.TrimSpace(p.token) == "" {
		return ChangeRequest{}, false, errors.New("github provider token is required")
	}
	...
	if owner == "" || name == "" {
		return ChangeRequest{}, false, errors.New("github repo is missing namespace/name")
	}
	...
	if head == "" || base == "" {
		return ChangeRequest{}, false, errors.New("github ensure open requires head/base")
	}
	...
	return ChangeRequest{}, false, fmt.Errorf("github create PR failed: %w", err)
}

func (p *GitHubProvider) Merge(ctx context.Context, repo ChangeRequestRepo, number int, input MergeInput) error {
	if strings.TrimSpace(p.token) == "" {
	...
	if err != nil {
		// GitHub docs: 405 means merge cannot be performed. Only treat it as success
		// if a follow-up GET confirms the PR is already merged.
...
func toChangeRequest(pr *ghapi.PullRequest) ChangeRequest {
	cr := ChangeRequest{}
	if pr == nil {
		return cr
	}
