...
type acpSessionKey struct {
	flowID  int64
	agentID string // v2 AgentProfile.ID
}
...
func (p *ACPSessionPool) Acquire(ctx context.Context, in acpSessionAcquireInput) (*pooledACPSession, *core.AgentContext, error) {
	...
	if err == core.ErrNotFound {
		...
		if cErr == nil {
			...
			slog.Warn("v2 acp pool: create agent context failed", "agent", in.Profile.ID, "flow_id", in.FlowID, "error", cErr)
			ac = nil
		}
	} else if err != nil {
		// Non-fatal; proceed without persistence.
		slog.Warn("v2 acp pool: find agent context failed", "agent", in.Profile.ID, "flow_id", in.FlowID, "error", err)
		ac = nil
	}
	...
	slog.Info("v2 acp pool: session acquired",
