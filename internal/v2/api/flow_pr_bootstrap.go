...
func (h *Handler) bootstrapPRFlow(w http.ResponseWriter, r *http.Request) {
	...
	title := fmt.Sprintf("ai-flow v2: flow %d", flowID)
...
func (h *Handler) currentPRFlowPrompts() engine.PRFlowPrompts {
	if h != nil && h.prPrompts != nil {
		return engine.MergePRFlowPrompts(h.prPrompts())
	}
	return engine.DefaultPRFlowPrompts()
}
