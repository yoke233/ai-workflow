...
type DecomposePlanner struct {
	chatFn func(ctx context.Context, projectID, systemPrompt, userMessage string) (string, error)
}

func NewDecomposePlanner(chatFn func(ctx context.Context, projectID, systemPrompt, userMessage string) (string, error)) *DecomposePlanner {
...
func (p *DecomposePlanner) Plan(ctx context.Context, projectID, prompt string) (*core.DecomposeProposal, error) {
	...
	reply, err := p.chatFn(ctx, projectID, decomposeSystemPrompt, prompt)
	if err != nil {
		return nil, err
	}
