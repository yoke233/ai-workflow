...
Secretary Agent 通过 ACP Client 建立持久多轮 session，天然支持对话上下文保持：

```
项目打开时:
  → acpClient = acpclient.New(agentConfig.LaunchCommand, agentConfig.LaunchArgs, agentConfig.Env)
  → acpClient.Initialize(clientCaps)
  → sessionID = acpClient.NewSession(project.RepoPath, mcpServers)
      // mcpServers 包含内嵌 MCP Server（暴露查询工具）
  → session 保持运行, 等待用户消息

用户每条消息:
  → result = acpClient.Prompt(sessionID, userMessage)
...
项目关闭 / 空闲超时:
  → acpClient.Close()
  → 对话历史已持久化到 Store
  → 下次可通过 acpClient.LoadSession(sessionID, cwd) 恢复（Agent 支持时）
```

### Agent 可切换

Secretary 默认使用 Claude，但可在配置中切换为其他 Agent：

```yaml
secretary:
  agent: claude          # 引用 agents 配置中的 name 字段
```

不同 Agent 只需配置不同的 launch_command，Secretary Layer 通过 ACP 协议统一通信，不感知具体 Agent 实现。

### 权限配置

Secretary Agent 的权限通过 ACP 三层权限模型控制，替代原有的 AllowedTools glob 模式：
...
acpClient.NewSession(projectDir, []MCPServerConfig{
...
type ChatSession struct {
    ID        string
    ProjectID string
    AgentSessionID string   // 关联的 Runtime session ID（运行时字段，不持久化）
...
- Agent session 是运行时状态。断线重连时通过 acpClient.LoadSession() 恢复已有 session（Agent 支持时），否则启动新 session
...
2. 后端检测到 Secretary 写文件（通过 tool_use 中的 Write/Edit 事件）
...
5. 后端 Plan Parser 处理：
   ├── 读取所有选中文件内容
   ├── 调用 AI（复用 Claude Driver）解析为结构化 TaskPlan + TaskItems
...
每个 Reviewer 和 Aggregator 都是一次 ACP 调用（NewSession → Prompt → 等 end_turn → 解析结果 → Close），只是 prompt 不同。不需要新的基础设施。与普通 Pipeline Stage 执行的区别：不创建 worktree、不写 Checkpoint、超时更短（默认 5 分钟）、capabilities 只需 fs_read。
