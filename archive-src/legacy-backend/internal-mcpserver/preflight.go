...
const legacySystemEventEndpointPath = "/api/v1/admin/ops/system-event"
...
func postSystemEvent(serverAddr, event string, data map[string]any) {
	...
	req, err := http.NewRequest(http.MethodPost, serverAddr+legacySystemEventEndpointPath, bytes.NewReader(body))
	if err != nil {
		return
	}
