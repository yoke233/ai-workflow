...
func runServer(ctx context.Context, args []string) error {
	...
	runtime, err := setupServerRuntime(ctx, exec, bootstrapSet, bus, launch.cfg)
	if err != nil {
		return err
	}
	runtimeActive := true
	...
	// --- V2 Engine Bootstrap ---
	_, _, runtimeManager, v2Cleanup, v2RouteRegistrar := bootstrapV2(expandStorePath(launch.cfg.Store.Path), bootstrapSet.RoleResolver, launch.cfg, launch.githubTokens, upgradeFunc)
	if v2Cleanup != nil {
		defer v2Cleanup()
	}
	...
	fmt.Printf(
		"Server started on %s (api: /api/v2, legacy ws: /api/v1/ws). Press Ctrl+C to stop.\n",
		launch.listenAddr,
	)
