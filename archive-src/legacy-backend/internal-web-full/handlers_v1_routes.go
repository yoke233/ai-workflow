...
// registerV1Routes registers archived legacy REST API routes mounted under /api/v1.
// Keep this isolated so the remaining compat whitelist can be pruned incrementally.
func registerV1Routes(
	...
	registerV1CompatRoutes(r, store, hub, webhookReplayer, restartFunc)
}

func registerV1CompatRoutes(
	r chi.Router,
	store core.Store,
	hub *Hub,
	webhookReplayer WebhookDeliveryReplayer,
...
func registerV1LegacyBusinessRoutes(
	r chi.Router,
	store core.Store,
	hub *Hub,
	issueManager IssueManager,
	...
	// Archived legacy business routes are intentionally left unmounted from the
	// default server runtime. Keep the implementation here temporarily so we can
	// migrate or remove it in smaller batches without losing source history.
