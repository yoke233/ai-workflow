...
function Start-Server {
  param([int]$Port)
  $stdout = Join-Path (Get-Location) (".tmp\\v2-pr-flow-" + $Port + ".out.log")
  $stderr = Join-Path (Get-Location) (".tmp\\v2-pr-flow-" + $Port + ".err.log")
  New-Item -ItemType Directory -Force -Path (Split-Path $stdout) | Out-Null

  $env:AI_WORKFLOW_V2_MOCK_EXECUTOR = "0"
...
try {
  ...
  $implement = Api -Method Post -Url "$base/flows/$flowId/steps" -Token $adminToken -Body @{
    ...
    max_retries = 3
    config = @{
      objective = "在该仓库中做一个最小变更：在 README.md 末尾追加一行 `\"v2 pr flow smoke $ts`\"。不要 git commit/push（后续步骤会处理）。如果存在 Go 测试则运行 go test ./...（失败则说明原因）。"
    }
  }
  ...
  $commit = Api -Method Post -Url "$base/flows/$flowId/steps" -Token $adminToken -Body @{
    ...
    config = @{
      ...
      commit_message = "test: v2 pr flow smoke $ts"
    }
  }
  ...
  $openPR = Api -Method Post -Url "$base/flows/$flowId/steps" -Token $adminToken -Body @{
    ...
    config = @{
      ...
      base = $BaseBranch
      title = "v2 pr flow smoke $ts"
      body  = "Automated PR created by v2 pr flow smoke."
    }
  }
